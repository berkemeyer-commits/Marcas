...
namespace Berke.Marcas.WebUI.Home
{
	...
	/// <summary>
	/// Summary description for MarcaDetalleL.
	/// </summary>
    /// 
    //join merge cambiar por join [merge]
	public partial class MarcaDetalleL : System.Web.UI.Page
	{
		#region Atributos
		...
		private void ValoresIniciales()
		{
			...
			#region Link para eliminar la marca
			...
			if( expe.Dat.TramiteID.AsInt == (int)GlobalConst.Marca_Tipo_Tramite.REGISTRO || expe.Dat.TramiteID.AsInt == (int)GlobalConst.Marca_Tipo_Tramite.RENOVACION ){
				if( Berke.Libs.Base.Acceso.OperacionPermitida("delm") ){
					lnkEliminar.Visible = true;
				}
			}
			#endregion Link para eliminar la marca

			#region Link para Modificar la Marca
			if( Berke.Libs.Base.Acceso.OperacionPermitida("upm") )
			{
				this.lblModif.Text = Berke.Libs.WebBase.Helpers.HtmlGW.OpenPopUp_Link(
					"MarcaModif.aspx","Modificar",ExpeID.ToString(),"ExpeID");
			}
			else
			{
				this.lblModif.Text = "";
			}
			#endregion Link para Modificar la Marca

			lblMarcaBasic.Text = Berke.Marcas.UIProcess.Model.Marca.BasicDataAsHTL( MarcaID );
			...
			lblExpedienteBasic.Text = Berke.Marcas.UIProcess.Model.Expediente.BasicDataAsHTL( ExpeID );

			this.lblInstrucciones.Text = Instrucciones();//Berke.Marcas.UIProcess.Model.ExpeInstruccion.ReadByExpeID_asHtml( ExpeID );
		...
		private string CambioEstado()
		{
			...
			return string.Format(boxPattern, "cambioEstadoBox", "divCambioEstado", tab.Html(), "Cambios de Estado");
...
//			string tmp = expeSit.Adapter.ReadAll_CommandString();
			...
			for( expeSit.GoTop(); ! expeSit.EOF; expeSit.Skip() )
			{
				...
				switch( trmSit.Dat.SituacionID.AsInt )
				{
					...
					case (int)GlobalConst.Situaciones.PRESENTADA : // Presentada
						...
						else if (expe.Dat.TramiteID.AsInt == (int) GlobalConst.Marca_Tipo_Tramite.REG_ADUANA)
						{
							hj = (int)GlobalConst.DocumentoTipo.PRES_REGADUANA;
						}
			...
			//lblHistorico.Text		= "<table class=\"tabla_jrk\"><tr><td class=\"td_header\"><a  onclick=\"setVisible('divSituaciones');\"><img id='img_divSituaciones' src=\"../Tools/imx/minus.bmp\" border=\"0\"></a> Histórico de Situaciones</td></tr><tr><td><div id='divSituaciones'>"+ tab.Html()+"</div></td></tr></table>";
		...
		#region Mostrar Merge
		private void mostrarMerge()
		{
		...
		#region Mostrar AvisoOpo
		private void mostrarAvisoOpo()
		{
			this.lblAvisosOpo.Text = AvisoOpo();
		
		}

		private string AvisoOpo()
		{
			Berke.Libs.Base.Helpers.AccesoDB db		= new Berke.Libs.Base.Helpers.AccesoDB();
			db.DataBaseName	= WebUI.Helpers.MyApplication.CurrentDBName;
			db.ServerName	= WebUI.Helpers.MyApplication.CurrentServerName;

			AvisoOpoTools	AvisoOpoTools =	new AvisoOpoTools();
			
			return (AvisoOpoTools.getAvisoOpoTable(db, this.MarcaID));
		}

		#endregion Mostrar AvisoOpo

		#region Mostrar Distribuidores
		...
		private string Distribuidores()
		{
			...
			Berke.DG.ViewTab.vExpedienteDistribuidor view = new Berke.DG.ViewTab.vExpedienteDistribuidor(db);
			view.ClearFilter();
			view.Dat.MarcaID.Filter = this.MarcaID;
			view.Adapter.ReadAll();
			...
			for( view.GoTop(); ! view.EOF; view.Skip())
			{
				...
				//tab.AddCell(  chkSpc( view.Dat.DistribuidorNombre.AsString + "(" + view.Dat.DistribuidorID.AsString + ")"));
				tab.AddCell(  chkSpc( view.Dat.Producto_Servicio.AsString ));
				tab.AddCell(  chkSpc(HtmlGW.Redirect_Link(view.Dat.DistribuidorID.AsString, view.Dat.DistribuidorNombre.AsString + "(" + view.Dat.DistribuidorID.AsString + ")",
			...
			txtFmt.Bold = true;
			txtFmt.Size = "-3";
			return string.Format(boxPattern, "distBox", "divDistribuidores", tab.Html(), "Distribuidores");
		}
		#endregion Mostrar Distribuidores
		...
		private void mostrarJerarquia()
		{
			...
			//this.pnlJerarquia.Visible = true;
			this.lblJerarquia.Text = Berke.Marcas.UIProcess.Model.Expediente.Jerarquia( ExpeID );
			this.lblJerarquia.Text = string.Format(boxPattern, "jerBox", "divJerarquia", lblJerarquia.Text, "Jerarqu&iacute;a de Expedientes");
		}
...
//			eliminarRegistro( "ExpedienteXPoder"			,"ExpedienteID", strExpeID, db );
		...
		protected void lnkEliminar_Click(object sender, System.EventArgs e)
		{
