...
namespace Berke.Marcas.WebUI.Home
{
	...
	public partial class CambioClienteTVS : System.Web.UI.Page
	{
		...
		protected void btnAsignarMarcas_Click(object sender, System.EventArgs e)
		{
			...
			#region CambioCliente_Read
				...
				Berke.DG.ViewTab.vMarcaClientePropietarioTVS vMCP_TVS =  Berke.Marcas.UIProcess.Model.CambioCliente.ReadTVS( inTB );
		...
		//	bool	inputError	= false;
			...
			dgMarcasAsignadas.DataSource = vMCP_TVS.Table;
			dgMarcasAsignadas.DataBind();

			Session["MarcasAsignadas"] = vMCP_TVS.Table;
			...
			{
				string scriptCliente= "<script language='javascript'>alert('Las actas ingresadas no pertenecen a trámites varios. Favor verifique.')</script>";
...
//			}
		...
		protected void btnGrabar_Click(object sender, System.EventArgs e)
		{
			...
			Berke.DG.ViewTab.vMarcaClientePropietarioTVS vMCP_TVS	= new Berke.DG.ViewTab.vMarcaClientePropietarioTVS((DataTable) Session["MarcasSeleccionadas"]);

			#region CambioCliente_Upsert

			foreach( DataGridItem item in this.dgConfirmacion.Items )
			{
				vMCP_TVS.Go(item.ItemIndex);
                vMCP_TVS.Edit();
				vMCP_TVS.Dat.ClienteID.Value = eccClienteNuevo.SelectedValue;
				vMCP_TVS.Dat.PropietarioID.Value = FuncionarioID; //FUNCIONARIO ID
				vMCP_TVS.Dat.Denominacion.Value = txtObsClientes.Text;//OBS
				vMCP_TVS.Dat.Registro.Value = lCorrID.Text;// CORRESPONDENCIA ID
				vMCP_TVS.PostEdit();
			}
			vMCP_TVS.AcceptAllChanges();
			#region Llamada al MODEL

			Berke.DG.ViewTab.ParamTab outTB =  Berke.Marcas.UIProcess.Model.CambioCliente.UpsertTVS( vMCP_TVS );
		...
		{
            Response.Redirect("CambioClienteTVS.aspx");
		}
		...
		protected void bConfirmar_Click(object sender, System.EventArgs e)
		{
			...
			Berke.DG.ViewTab.vMarcaClientePropietarioTVS vMCP_TVS	= new Berke.DG.ViewTab.vMarcaClientePropietarioTVS((DataTable) Session["MarcasAsignadas"]);
...
//				}
			...
			dgConfirmacion.DataSource = vMCP_TVS.Table;
			dgConfirmacion.DataBind();
			Session["MarcasSeleccionadas"]= vMCP_TVS.Table;
