...
{
	/// <summary>
	/// Descripción breve de IngresarLedes1998BI.
	/// </summary>
	/// 
	...
	public partial class IngresarLedes1998BI : System.Web.UI.Page
	{
		#region Properties
		...
		private void ConfigurarGrilla()
		{
			...
			dgw.AddText( "tbItemTaxTotal"	,"Item Tax Total", 60);
			dgw.AddText( "tbItemTaxRate"	,"Item Tax Rate", 60);
			dgw.AddText( "tbItemTaxType", "Item Tax Type", 35);
		}
		...
		private void CargarCabecera(Berke.Libs.Base.Helpers.AccesoDB db)
		{
			...
			tbPoNumber.Text = LedesFactura.Dat.po_number.AsString.Trim();
			tbClientTaxID.Text = LedesFactura.Dat.client_tax_id.AsString.Trim();
			tbMatterName.Text = LedesFactura.Dat.matter_name.AsString.Trim();
			tbInvoiceTaxTotal.Text = String.Format("{0:0.00}", LedesFactura.Dat.invoice_tax_total.AsDecimal);
			tbInvoiceNetTotal.Text = String.Format("{0:0.00}", LedesFactura.Dat.invoice_net_total.AsDecimal);
			tbInvoiceCurrency.Text = LedesFactura.Dat.invoice_currency.AsString.Trim();
			tbTimekeeperLastname.Text = LedesFactura.Dat.timekeeper_last_name.AsString.Trim();
			tbTimekeeperFirstname.Text = LedesFactura.Dat.timekeeper_first_name.AsString.Trim();
			ddlAccountType.SelectedValue = LedesFactura.Dat.account_type.AsString;
			tbLawFirmName.Text = LedesFactura.Dat.law_firm_name.AsString.Trim();
			tbLawFirmAddress1.Text = LedesFactura.Dat.law_firm_address_1.AsString.Trim();
			tbLawFirmAddress2.Text = LedesFactura.Dat.law_firm_address_2.AsString.Trim();
			tbLawFirmCity.Text = LedesFactura.Dat.law_firm_city.AsString.Trim();
			tbLawFirmStateorRegion.Text = LedesFactura.Dat.law_firm_stateorregion.AsString.Trim();
			tbLawFirmPostcode.Text = LedesFactura.Dat.law_firm_postcode.AsString.Trim();
			tbLawFirmCountry.Text = LedesFactura.Dat.law_firm_country.AsString.Trim();
			tbClientName.Text = LedesFactura.Dat.client_name.AsString.Trim();
			tbClientAddress1.Text = LedesFactura.Dat.client_address_1.AsString.Trim();
			tbClientAddress2.Text = LedesFactura.Dat.client_address_2.AsString.Trim();
			tbClientCity.Text = LedesFactura.Dat.client_city.AsString.Trim();
			tbClientStateorRegion.Text = LedesFactura.Dat.client_stateorregion.AsString.Trim();
			tbClientPostcode.Text =	LedesFactura.Dat.client_postcode.AsString.Trim();
			tbClientCountry.Text =LedesFactura.Dat.client_country.AsString.Trim();
		}
		...
		private void CargarDetalles(Berke.Libs.Base.Helpers.AccesoDB db)
		{
			...
			foreach (DataGridItem item in GridLedesDetalle.Items) 
			{
				...
				dgw.Set(item, "tbItemTaxRate", String.Format("{0:0.00}", LedesDetalle.Dat.line_item_tax_rate.AsDecimal));
				//dgw.Set(item, "tbItemTaxRate", LedesDetalle.Dat.line_item_tax_rate.AsString);
				dgw.Set(item, "tbItemTaxTotal", String.Format("{0:0.00}", LedesDetalle.Dat.line_item_tax_total.AsDecimal));
				dgw.Set(item, "tbItemTaxType", LedesDetalle.Dat.line_item_tax_type.AsString);
				LedesDetalle.Skip();
			}
			GridDataTable = LedesDetalle.Table;
		}
		...
		protected void btnGrabar_Click(object sender, System.EventArgs e)
		{
			...
			try 
			{
				...
				url.redirect( "IngresarLedes1998BI.aspx" );
			} 
		...
		{			
			...
			try 
			{
				LedesFactura.Dat.invoice_date.Value = Convert.ToDateTime(tbInvoiceDate.Text);
			} 
			catch 
			{
				msg = "Formato inválido para el campo Invoice date";
				return msg;
			}
			//Validar billing_start_date
			try 
			{
				LedesFactura.Dat.billing_start_date.Value = Convert.ToDateTime(tbBillingStartDate.Text);
			} 
			catch 
			{
				msg = "Formato inválido para el campo Billing start date";
				return msg;
			}
			//Validar billing_end_date
			try 
			{
				LedesFactura.Dat.billing_end_date.Value = Convert.ToDateTime(tbBillingEndDate.Text);				
			} 
			catch 
			{
				msg = "Formato inválido para el campo Billing end date";
				return msg;
			}
			//Validar invoice_total
			try 
			{
				LedesFactura.Dat.invoice_total.Value = Convert.ToDecimal(tbInvoiceTotal.Text);
			} 
            catch 
			{
				msg = "Formato inválido para el campo Invoice Total";
				return msg;
			}
			//LedesFactura.Dat.client_tax_id.Value = tbClientTaxID.Text;
			if (tbClientTaxID.Text == "")
			{
				msg = "El campo Client Tax ID no puede ser nulo.";
				return msg;
			}	
			LedesFactura.Dat.client_tax_id.Value = tbClientTaxID.Text;

			//LedesFactura.Dat.matter_name.Value = tbMatterName.Text;
			if (tbMatterName.Text == "")
			{
				msg = "El campo Matter Name no puede ser nulo.";
				return msg;
			}	
			LedesFactura.Dat.matter_name.Value = tbMatterName.Text;

			//LedesFactura.Dat.invoice_currency.Value = tbInvoiceCurrency.Text;
			if (tbInvoiceCurrency.Text == "")
			{
				msg = "El campo Invoice Currency no puede ser nulo.";
				return msg;
			}	
			LedesFactura.Dat.invoice_currency.Value = tbInvoiceCurrency.Text;

			//LedesFactura.Dat.account_type.Value = ddlAccountType.SelectedValue;
			if (ddlAccountType.SelectedValue == "")
			{
				msg = "El campo Account Type no puede ser nulo.";
				return msg;
			}	
			LedesFactura.Dat.account_type.Value = ddlAccountType.SelectedValue;
			...
			LedesFactura.Dat.po_number.Value = tbPoNumber.Text;
			
			
			LedesFactura.Dat.invoice_tax_total.Value = tbInvoiceTaxTotal.Text;
			LedesFactura.Dat.invoice_net_total.Value = tbInvoiceNetTotal.Text;
			
			LedesFactura.Dat.timekeeper_last_name.Value = tbTimekeeperLastname.Text;
			LedesFactura.Dat.timekeeper_first_name.Value = tbTimekeeperFirstname.Text;
			
			LedesFactura.Dat.law_firm_name.Value = tbLawFirmName.Text;
			LedesFactura.Dat.law_firm_address_1.Value = tbLawFirmAddress1.Text;
			LedesFactura.Dat.law_firm_address_2.Value = tbLawFirmAddress2.Text;
			LedesFactura.Dat.law_firm_city.Value = tbLawFirmCity.Text;
			LedesFactura.Dat.law_firm_stateorregion.Value = tbLawFirmStateorRegion.Text;
			LedesFactura.Dat.law_firm_postcode.Value = tbLawFirmPostcode.Text;
			LedesFactura.Dat.law_firm_country.Value = tbLawFirmCountry.Text;
			LedesFactura.Dat.client_name.Value = tbClientName.Text;
			LedesFactura.Dat.client_address_1.Value = tbClientAddress1.Text;
            LedesFactura.Dat.client_address_2.Value = tbClientAddress2.Text;
			LedesFactura.Dat.client_city.Value = tbClientCity.Text;
			LedesFactura.Dat.client_stateorregion.Value = tbClientStateorRegion.Text;
			LedesFactura.Dat.client_postcode.Value = tbClientPostcode.Text;
			LedesFactura.Dat.client_country.Value = tbClientCountry.Text;
		...
		private string GrabarDetalle(Berke.Libs.Base.Helpers.AccesoDB db)
		{
			...
			foreach( DataGridItem item in GridLedesDetalle.Items ) 
			{
				if (dgw.GetText( item, "tbItemNumber" ) != "") 
				{
					...
					try 
					{
						item_number = Convert.ToInt32(dgw.GetText( item, "tbItemNumber" ));
					} 
					catch 
					{
						msg = "Formato inválido para el campo Item Number. Linea #" + dgw.GetText( item, "tbItemNumber" );
						return msg;
					}
					...
					try 
					{
						LedesDetalle.Dat.line_item_unit_cost.Value = dgw.GetText( item, "tbUnitCost" );
					} 
					catch 
					{
						msg = "Formato inválido para el campo UnitCost. Linea #" + item_number.ToString();
						return msg;
					}
					//Validar line_item_date
					try 
					{
						LedesDetalle.Dat.line_item_date.Value = Convert.ToDateTime(dgw.GetText( item, "tbDate" ));
					} 
					catch 
					{
						msg = "Formato inválido para el campo Item Date. Linea #" + item_number.ToString();
						return msg;
					}
					...
					try 
					{
						LedesDetalle.Dat.line_item_total.Value = dgw.GetText( item, "tbTotal" );
					} 
					catch 
					{
						msg = "Formato inválido para el campo Item Total. Linea #" + item_number.ToString();
						return msg;
					}
					...
					try 
					{
						LedesDetalle.Dat.line_item_number_of_units.Value = dgw.GetText( item, "tbNumberUnits" );
					} 
					catch 
					{
						msg = "Formato inválido para el campo Number of Units. Linea #" + item_number.ToString();
						return msg;
					}
					#endregion Validar ingreso de campos no nulos
					...
					LedesDetalle.Dat.line_item_tax_rate.Value = dgw.GetText( item, "tbItemTaxRate" );
					LedesDetalle.Dat.line_item_tax_total.Value = dgw.GetText( item, "tbItemTaxTotal" );
					LedesDetalle.Dat.line_item_tax_type.Value = dgw.GetText( item, "tbItemTaxType" );
		...
		private string ValidarDatos() 
		{
			...
			decimal monto_total_taxes_detalles = 0;
			...
			else if (LedesFactura.Dat.client_tax_id.AsString == "") 
			{
				msg = "Debe completar un valor para el campo Client Tax Id";
				return msg;
			}
			else if (LedesFactura.Dat.matter_name.AsString == "") 
			{
				msg = "Debe completar un valor para el campo Matter Name";
				return msg;
			}
			else if (LedesFactura.Dat.invoice_net_total.AsString == "") 
			{
				msg = "Debe completar un valor para el campo Invoice Net Total";
				return msg;
			}
			else if (LedesFactura.Dat.invoice_currency.AsString == "") 
			{
				msg = "Debe completar un valor para el campo Invoice Currency";
				return msg;
			}
			else if (LedesFactura.Dat.timekeeper_last_name.AsString.Trim() == "" && LedesFactura.Dat.timekeeper_name.AsString != "") 
			{
				msg = "Debe completar un valor para el campo Timekeeper Lastname";
				return msg;
			}
			else if (LedesFactura.Dat.timekeeper_first_name.AsString.Trim() == "" && LedesFactura.Dat.timekeeper_name.AsString != "") 
			{
				msg = "Debe completar un valor para el campo Timekeeper Firstname";
				return msg;
			}
			else if (LedesFactura.Dat.account_type.AsString.Trim() == "") 
			{
				msg = "Debe completar un valor para el campo Account Type";
				return msg;
			}
			...
			for (LedesDetalle.GoTop(); ! LedesDetalle.EOF; LedesDetalle.Skip()) 
			{
				#region Validar Item Tax Rate
				if (!(LedesDetalle.Dat.line_item_tax_rate.AsDecimal >= 0 && LedesDetalle.Dat.line_item_tax_rate.AsDecimal <= 1))
				{
					msg = "El valor de Item Tax Rate en la linea con ID " + LedesDetalle.Dat.line_item_number.AsString + " es incorrecto. Debe ser un valor comprendido entre 0 y 1)";
					return msg;
				}
				#endregion Validar Item Tax Rate

				decimal monto_linea_item_total = 0;
				decimal monto_linea_item_tax = 0;

				//monto_total_detalles = monto_total_detalles + LedesDetalle.Dat.line_item_total.AsDecimal;

				#region Verificar line_item_tax_total
				monto_linea_item_tax = ((LedesDetalle.Dat.line_item_unit_cost.AsDecimal * LedesDetalle.Dat.line_item_number_of_units.AsDecimal) +
									   LedesDetalle.Dat.line_item_adjustment_amount.AsDecimal) * LedesDetalle.Dat.line_item_tax_rate.AsDecimal;

				if (LedesDetalle.Dat.line_item_tax_total.AsDecimal != monto_linea_item_tax)
				{
					msg = "El valor de Item Tax Total es incorrecto en la linea con ID " + LedesDetalle.Dat.line_item_number.AsString + ". " +
						  "Se debe aplicar la formula siguiente: " + 
						   "Item Tax Total = ((Item Unit Cost \\* Item Number of Units) + Item Adjustment Amount) \\* Item Tax Rate. " +
						  "El valor debería ser: " + String.Format("{0:0.00}", monto_linea_item_tax);
					return msg;
				}
				#endregion Verificar line_item_tax_total
				
				#region Verificar line_item_total
				monto_linea_item_total = (LedesDetalle.Dat.line_item_unit_cost.AsDecimal * LedesDetalle.Dat.line_item_number_of_units.AsDecimal) +
										 LedesDetalle.Dat.line_item_adjustment_amount.AsDecimal + monto_linea_item_tax;

				if (LedesDetalle.Dat.line_item_total.AsDecimal != monto_linea_item_total)
				{
					msg = "El valor de Item Total es incorrecto en la linea con ID " + LedesDetalle.Dat.line_item_number.AsString + ". " +
						  "Se debe aplicar la formula siguiente: " + 
						   "Item Total = (Item Unit Cost \\* Item Number of Units) + Item Adjustment Amount + Item Tax Total. " + 
						  "El valor debería ser: " + String.Format("{0:0.00}", monto_linea_item_total);
					return msg;
				}
				#endregion Verificar line_item_total

				monto_total_detalles = monto_total_detalles + monto_linea_item_total;
				monto_total_taxes_detalles = monto_total_taxes_detalles + monto_linea_item_tax;
				...
				else if (LedesDetalle.Dat.line_item_tax_total.AsString.Trim() == "") 
				{
					msg = "Debe completar un valor para el campo Item Tax Total en la linea " + LedesDetalle.Dat.line_item_number.AsString;
					return msg;
				}
			...
			#region Validar invoice_tax_total
			//Controlar que total de la cabecera coincida con total de detalles
			if (LedesFactura.Dat.invoice_tax_total.AsDecimal != monto_total_taxes_detalles)
			{
				msg = "La suma de los Item Tax Total no coincide con Invoice Tax Total";
				return msg;
			}
			#endregion Validar invoice_tax_total

			#region Validar invoice_net_total
			//Controlar que total de la cabecera coincida con total de detalles
			if (LedesFactura.Dat.invoice_net_total.AsDecimal != (monto_total_detalles - monto_total_taxes_detalles)) 
			{
				msg = "El valor de Invoice Net Total es incorrecto. Debe aplicar la formula: (Invoice Total - Invoice Tax Total).";
				return msg;
			}
			#endregion Validar invoice_net_total
...
";
			...
			{				
				cuerpo = cuerpo + LedesFactura.Dat.invoice_date.AsDateTime.ToString("yyyyMMdd") + @"|" +
					...
					LedesFactura.Dat.po_number.AsString.Trim() + @"|" +
					LedesFactura.Dat.client_tax_id.AsString.Trim() + @"|" +
					LedesFactura.Dat.matter_name.AsString.Trim() + @"|" +
					String.Format("{0:0.00}", LedesFactura.Dat.invoice_tax_total.AsDecimal).Replace(",", ".") + @"|" +
					String.Format("{0:0.00}", LedesFactura.Dat.invoice_net_total.AsDecimal).Replace(",", ".") + @"|" +
					LedesFactura.Dat.invoice_currency.AsString.Trim() + @"|" +
					LedesFactura.Dat.timekeeper_last_name.AsString.Trim() + @"|" +
					LedesFactura.Dat.timekeeper_first_name.AsString.Trim() + @"|" +
					LedesFactura.Dat.account_type.AsString.Trim() + @"|" +
					LedesFactura.Dat.law_firm_name.AsString.Trim() + @"|" +
					LedesFactura.Dat.law_firm_address_1.AsString.Trim() + @"|" +
					LedesFactura.Dat.law_firm_address_2.AsString.Trim() + @"|" +
					LedesFactura.Dat.law_firm_city.AsString.Trim() + @"|" +
					LedesFactura.Dat.law_firm_stateorregion.AsString.Trim() + @"|" +
					LedesFactura.Dat.law_firm_postcode.AsString.Trim() + @"|" +
					LedesFactura.Dat.law_firm_country.AsString.Trim() + @"|" +
					LedesFactura.Dat.client_name.AsString.Trim() + @"|" +
					LedesFactura.Dat.client_address_1.AsString.Trim() + @"|" +
					LedesFactura.Dat.client_address_2.AsString.Trim() + @"|" +
					LedesFactura.Dat.client_city.AsString.Trim() + @"|" +
					LedesFactura.Dat.client_stateorregion.AsString.Trim() + @"|" +
					LedesFactura.Dat.client_postcode.AsString.Trim() + @"|" +
					LedesFactura.Dat.client_country.AsString.Trim() + @"|" +
					String.Format("{0:0.00}", LedesDetalle.Dat.line_item_tax_rate.AsDecimal).Replace(",", ".") + @"|" +
					String.Format("{0:0.00}", LedesDetalle.Dat.line_item_tax_total.AsDecimal).Replace(",", ".") + @"|" +
					LedesDetalle.Dat.line_item_tax_type.AsString.Trim() + @"[]";
			}
		...
		protected void GridLedesDetalle_SelectedIndexChanged(object sender, System.EventArgs e)
		{
